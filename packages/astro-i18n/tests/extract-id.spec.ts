...
describe('extractCleanId', () => {
  ...
  test('throws when unknown locale prefix is present', () => {
    expect(() => extractCleanId('fr/2025-01-22-my-post', allowedLocales)).toThrow('Invalid language: fr')
  })
  ...
  test('throws on locale from middle of path', () => {
    expect(() => extractCleanId('category/es/2025-01-22-post', allowedLocales)).toThrow('Invalid language: category')
  })
